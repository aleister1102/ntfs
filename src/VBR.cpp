...
void print(NTFS_Partition_Boot_Sector ntfs) {
    ...
    cout << "Total sectors: "; printInfomation(ntfs.total_sector);
    cout << "Logical cluster number for MFT: "; printInfomation(ntfs.logical_cluster_number_mft);
    cout << "Logical cluster number for MFTMirr: "; printInfomation(ntfs.logical_cluster_number_mftmirr);
...
LONG convertBase2ToBase10(string s, bool part) { //part is low -> 0 or high -> 1
    LONG result = 0;
    string substr;
    if (part == 0) {
...
void getAttribute(MFT_Entry& entry, BYTE* sector) {
    ...
    if (type.base10_value == 48) { // File Name 

    }
    else if (type.base10_value == 128) { // Data 
...
void printMFT(MFT_Entry entry) {
    cout << "Signature: "; printInfomation(entry.signature);
    cout << "Offset First Attribute: "; printInfomation(entry.offset_first_attribute);
